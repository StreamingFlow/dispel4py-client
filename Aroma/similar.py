...
def print_similar_and_completions(query_record, records, vectorizer, counter_matrix):
    ...
    #     ast_to_code(records[query_record["index"]]["ast"])
    ...
    if config.PRINT_SIMILAR:
        ...
        #    print(ast_to_code(candidate_record["ast"]))
            #ast_to_code(candidate_record["ast"])
            similarPEs.append([candidate_record['peId'], candidate_record['peName'], score, pruned_score, ast_to_code(candidate_record["ast"])])
        #    if config.PRINT_PRUNED:
                #print(
                #    f"------------------- similar code (pruned) ------------------ score = {pruned_score}"
                #)
                #print(ast_to_code(pruned_record["ast"]))
        #        ast_to_code(pruned_record["ast"])
