...
def test_semantic_returns_pe_objects_and_prints_metadata_once(client_classes, monkeypatch):
    ...
    web.user_login_id = "rosa"
    web._requestJson = Mock(return_value=[dict(peId=9, peName="Average", description="average",
    ...
    result = client.searchRegistrySemantic("average", "pe")
...
def test_empty_structural_matches_do_not_sort_missing_columns(client_classes, monkeypatch):
    ...
    web.user_login_id = "rosa"
    assert web._astPEResult([], []) == []
    assert web._astWorkflowResults([]) == []
    assert web._searchAST([], "def f(x): return x", "pe") == []
