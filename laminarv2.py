...
class LaminarCLI(cmd.Cmd):
    def __init__(self):
        super().__init__()
        self.prompt = "(laminar) "
        self.intro = """Welcome to the Laminar CLI. Type 'help' for a list of available commands."""
    ...
    def help_run(self):
        ...
        print("Usage: run [workflow_identifier] [--rawinput] [-v] [-i INPUT] [-r RESOURCE] [--multi] [--dynamic]")
    ...
    def do_get_pe(self, arg):
        parser = CustomArgumentParser(exit_on_error=False)
        parser.add_argument("pe_identifier")
        parser.add_argument("--describe", action="store_true")
        try:
            args = vars(parser.parse_args(shlex.split(arg)))
            pe = client.get_PE(args["pe_identifier"], describe=args["describe"])
            print(pe)
        except argparse.ArgumentError as e:
            print(e.message.replace("laminar.py", "get_pe"))

    def help_get_pe(self):
        print("Retrieves a PE by its name or ID and optionally provides a description")
        print("Usage: get_pe [pe_identifier] [--describe]")

    def do_get_workflow(self, arg):
        parser = CustomArgumentParser(exit_on_error=False)
        parser.add_argument("workflow_identifier")
        parser.add_argument("--describe", action="store_true")
        try:
            args = vars(parser.parse_args(shlex.split(arg)))
            workflow = client.get_Workflow(args["workflow_identifier"], describe=args["describe"])
            print(workflow)
        except argparse.ArgumentError as e:
            print(e.message.replace("laminar.py", "get_workflow"))

    def help_get_workflow(self):
        print("Retrieves a workflow by its name or ID and optionally provides a description")
        print("Usage: get_workflow [workflow_identifier] [--describe]")

    def do_get_pes_by_workflow(self, arg):
        parser = CustomArgumentParser(exit_on_error=False)
        parser.add_argument("workflow_identifier")
        try:
            args = vars(parser.parse_args(shlex.split(arg)))
            pes = client.get_PEs_By_Workflow(args["workflow_identifier"])
            for pe in pes:
                print(pe)
        except argparse.ArgumentError as e:
            print(e.message.replace("laminar.py", "get_pes_by_workflow"))

    def help_get_pes_by_workflow(self):
        print("Retrieves all PEs in a specified workflow")
        print("Usage: get_pes_by_workflow [workflow_identifier]")

    def do_describe(self, arg):
        parser = CustomArgumentParser(exit_on_error=False)
        parser.add_argument("object_identifier")
        try:
            args = vars(parser.parse_args(shlex.split(arg)))
            obj = client.get_PE(args["object_identifier"])
            if obj is None:
                obj = client.get_Workflow(args["object_identifier"])
            if obj is not None:
                client.describe(obj)
            else:
    ...
    def help_describe(self):
        print("Describes a PE or workflow by its name or ID")
        print("Usage: describe [object_identifier]")
